...
def conv2d_simple(x, W, b=None, stride=1, pad=0):
    ...
    col = im2col(x, (KH, KW), stride, pad, to_matrix=True)  # 展开数据
    Weight = Weight.reshape(OC, -1).transpose()  # 将卷积核展开(-1表示自动计算)
    t = linear(col, Weight, b)  # 矩阵乘法
    y = t.reshape(N, OH, OW, OC).transpose(0, 3, 1, 2)  # 转换形状
    return y
...
class Pooling(Function):
    ...
    def forward(self, x):
        ...
        self.indexes = col.argmax(axis=2)  # 已经展开，直接得到一行中最大值的索引
        y = col.max(axis=2)
        return y
